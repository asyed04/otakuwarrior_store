#!/usr/bin/env ruby
# This script forcefully updates product images with direct URLs
# Run it with: rails runner force_update_images.rb

require 'open-uri'

puts "Starting FORCED image update for all products..."
...
Product.find_each.with_index do |product, index|
  begin
    puts "Forcefully updating image for product #{product.id}: #{product.name}"
...
puts "✅ Finished FORCED update of all product images"
