class CheckoutController < ApplicationController
  ...
  def new
    ...
    flash.now[:notice] = "Page refreshed at #{Time.zone.now.strftime('%H:%M:%S')}"
  end
  ...
  def confirm
    ...
    # Format for display
    @subtotal_display = format('$%.2f', @subtotal)
    @gst_display = format('$%.2f', gst)
    @pst_display = format('$%.2f', pst)
    @hst_display = format('$%.2f', hst)
    @total_display = format('$%.2f', @total)
  ...
  def latest_invoice
    ...
    @order = if customer_signed_in?
               current_customer.orders.order(created_at: :desc).first
             else
               # For guests, try to find the order from the session
               Order.find_by(id: session[:last_order_id])
             end
  ...
  def create
    ...
    Stripe.api_key = Rails.application.credentials.dig(:stripe, :secret_key) if Stripe.api_key.blank?
